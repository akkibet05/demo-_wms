...
router.post("/view/supervisors_settings", auth,  async(req, res) => {
    try {
        const {rm_name, rm_email, rm_mobilenum} = req.body
        const supervisor = await supervisor_settings.findOne()
       
        if (supervisor == null) {
            
            const data = new supervisor_settings({RMSName : rm_name, RMSEmail : rm_email, RMSnumber : rm_mobilenum})
            await data.save()
            
        } else {
            
            supervisor.RMSName = rm_name
            supervisor.RMSEmail = rm_email
            supervisor.RMSnumber = rm_mobilenum
